...
*
* **Test Procedure:**
* Refer to UT specification documentation [tv-settings_L2_Low_Level_Test_Spec.md](../docs/pages/tv-settings_L2_Low_Level_Test_Spec.md)
*/
...
void test_l2_tvSettings_GetSupportedVideoFormats(void)
{
    ...
    tvVideoFormatType_t *videoFormats[VIDEO_FORMAT_MAX];
    unsigned short numberOfFormats = 0;
    ...
    UT_ASSERT_KVP_EQUAL_PROFILE_UINT16(numberOfFormats, "tvSettings/VideoFormat/numberOfFormats");

    pInstance = ut_kvp_profile_getInstance();
    count = ut_kvp_getListCount(pInstance,"tvSettings/VideoFormat/index");

    for ( int i = 0; i < count ; i++ )
    {
        flag = 0;
        snprintf(keyValue, KEY_VALUE_SIZE, "tvSettings/VideoFormat/index/%d", i);
        format = ut_kvp_getUInt32Field( pInstance, keyValue);
        j = 0;
        ...
        else
        {
            ...
            UT_LOG_ERROR("Video Format : %d is not in the list of supported VideoFormats ", *videoFormats[j]);
        }
    }
...
void test_l2_tvSettings_VerifyFrameRateWhenStopped(void)
{
    ...
    {
        UT_LOG_ERROR("Invoking TvTerm due to failure of GetCurrentVideoFrameRate");
    }
...
void test_l2_tvSettings_GetTVSupportedVideoSources(void)
{
    ...
    if (status != tvERROR_NONE)
    {
        UT_LOG_ERROR("Invoking TvTerm due to failure in GetTVSupportedVideoSources");
    }

    UT_ASSERT_TRUE(numberOfSources >= 1 && numberOfSources <= VIDEO_SOURCE_MAX);

    UT_ASSERT_KVP_EQUAL_PROFILE_UINT16(numberOfSources, "tvSettings/VideoSource/numberOfSources");

    pInstance = ut_kvp_profile_getInstance();
    count = ut_kvp_getListCount(pInstance,"tvSettings/VideoSource/index");

    for (int i=0; i < count; i++ )
    {
        flag = 0;
        snprintf(keyValue, KEY_VALUE_SIZE, "tvSettings/VideoSource/index/%d", i);
        source = ut_kvp_getUInt32Field( pInstance, keyValue);
        j = 0;
        ...
        else
        {
            ...
            UT_LOG_DEBUG("Video Source : %d is not in the list of supported VideoSources ", videoSources[j] ? *videoSources[j] : 0);
        }
    }
...
void test_l2_tvSettings_VerifyNoVideoSource(void)
{
    ...
    {
        UT_LOG_ERROR("Invoking TvTerm due to failure of GetCurrentVideoSource");
    }
...
void test_l2_tvSettings_SetAndGetBacklight(void)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (status != tvERROR_NONE)
        {
            ...
            UT_LOG_ERROR("Invoking TvTerm() due to failure of SetBacklight()");
        }
...
void test_l2_tvSettings_SetAndGetBacklightFade(void)
{
    ...
    int to = 0;
    int duration = 0;
    int get_from = 0, get_to = 0, get_current = 0;
    srand(time(NULL));
    ...
    for(int i= 0 ; i < 4; i++ )
    {
        ...
        if (status != tvERROR_NONE)
        {
            UT_LOG_ERROR("Invoking TvTerm() due to failure in SetBacklightFade()");
        }
...
void test_l2_tvSettings_GetSupportedBacklightModes(void)
{
    ...
    if (status != tvERROR_NONE)
    {
        UT_LOG_ERROR("Invoking TvTerm() due to failure in GetSupportedBacklightModes()");
    }

    pInstance = ut_kvp_profile_getInstance();
    count = ut_kvp_getListCount(pInstance,"tvSettings/BacklightControl/index");
    for (int i = 0; i < count; i++)
    {
        flag = 0;
        snprintf(keyValue, KEY_VALUE_SIZE, "tvSettings/BacklightControl/index/%d", i);
        blModes = ut_kvp_getUInt32Field( pInstance, keyValue);
        for (int j = 0; j < 6; j++ )
        {
            mode = (getblModes >> j) & 1;
            if ( mode == blModes )
...
void test_l2_tvSettings_SetAndGetBacklightMode(void)
{
    ...
    for(setMode = tvBacklightMode_NONE; setMode < tvBacklightMode_MAX ; setMode++)
    {
        ...
        if (ret != tvERROR_NONE)
        {
            UT_LOG_ERROR("Invoking TvTerm() due to failure in SetCurrentBacklightMode()");
        }
...
void test_l2_tvSettings_GetSupportedDimmingModes(void)
{
    ...
    unsigned short numDimmingModes = 0;
    ut_kvp_instance_t *pInstance = NULL;
    char keyValue[KEY_VALUE_SIZE] = {0};
    uint32_t mode = 0;
    int flag = 0;
    ...
    pInstance = ut_kvp_profile_getInstance();
    count = ut_kvp_getListCount(pInstance,"tvSettings/DimmingMode/index");
    for ( int i = 0; i < count; i++ )
    {
        flag = 0;
        snprintf(keyValue, KEY_VALUE_SIZE, "tvSettings/DimmingMode/index/%d", i);
        mode = ut_kvp_getUInt32Field( pInstance, keyValue);
        j = 0;

        for ( j = 0; j < numDimmingModes; j++ )
        {
            if ( dimmingModes[j] && (*dimmingModes[j] == mode) )
            {
                flag = 1;
                break;
            }
        ...
        if (flag == 1)
        {
            ...
            UT_LOG_DEBUG("dimmingMode: %d is in the list of supported dimmingModes",  dimmingModes[j]? *dimmingModes[j]: 0);
        }
        else
        {
            UT_FAIL("dimmingMode mismatch");
            UT_LOG_DEBUG("dimmingModes : %d is not in the list of supported dimmingModes ", dimmingModes[j]? *dimmingModes[j]: 0);
        }
    }
...
void test_l2_tvSettings_SetAndGetBrightness(void)
{
    ...
    for (int i=0;i<4;i++)
    {
        ...
        if (status != tvERROR_NONE)
        {
            ...
            UT_LOG_ERROR("Invoking TvTerm due to SetBrightness failure");
        }
...
void test_l2_tvSettings_SetAndGetContrast(void)
{
    ...
    for ( int i = 0; i < 4 ; i++ )
    {
        ...
        if (ret != tvERROR_NONE)
        {
            ...
            UT_LOG_ERROR("Invoking TvTerm due to SetContrast failure");
        }
...
void test_l2_tvSettings_SetAndGetColorTemperature(void)
{
    ...
    for ( colorTemp = tvColorTemp_STANDARD; colorTemp < tvColorTemp_MAX; colorTemp++ )
    {
        ...
        if (status != tvERROR_NONE)
        {
            ...
            UT_LOG_ERROR("Invoking TvTerm due to failure of SetColorTemperature");
        }
...
void test_l2_tvSettings_GetSupportedDolbyVisionModes(void)
{
    ...
    UT_ASSERT_KVP_EQUAL_PROFILE_UINT16(count, "tvSettings/DolbyVisionMode/numberOfDolbyVisionMode");

    pInstance = ut_kvp_profile_getInstance();
    getcount = ut_kvp_getListCount(pInstance,"tvSettings/DolbyVisionMode/index");
    for (int i = 0; i < getcount; i++ )
    {
        flag = 0;
        snprintf(keyValue, KEY_VALUE_SIZE, "DolbyVisionMode/index/%d", i);
        mode = ut_kvp_getUInt32Field( pInstance, keyValue);
        j = 0;
        for ( j = 0; j < count; j++)
        {
            if ( dvModes[j] && ( *dvModes[j] == mode ))
            {
                flag = 1;
                break;
            }
        ...
        if (flag == 1)
        {
            ...
            UT_LOG_DEBUG("DolbyVision Mode : %d is in the list of supported DolbyVisionModes", dvModes[j]? *dvModes[j] : 0);
        }
        else
        {
            UT_FAIL("DolbyVisionMode mismatch");
            UT_LOG_DEBUG("DolbyVisionMode : %d is not in the list of supported DolbyVisionModes ", dvModes[j]? *dvModes[j] : 0);
        }
    }
...
void test_l2_tvSettings_SetAndGetDolbyVisionMode(void)
{
    ...
    tvError_t status = tvERROR_NONE;
    tvDolbyMode_t dolbyMode = tvMode_Max;
...
void test_l2_tvSettings_GetTVSupportedPictureModes(void)
{
    ...
    pInstance = ut_kvp_profile_getInstance();
    getcount = ut_kvp_getListCount(pInstance,"tvSettings/PictureMode/index");
    for (int i=0;i<getcount;i++)
    {
        flag = 0;
        ...
        for ( j = 0; j < count; j++)
        {
            if ( pictureModes[j] && ( pictureModes[j]->value == mode ) )
            {
                flag = 1;
                break;
            }
        ...
        if (flag == 1)
        {
            ...
            UT_LOG_DEBUG("Picture Mode : %d is in the list of supported PictureMode", pictureModes[j] ? pictureModes[j]->value : 0 );
        }
        else
        {
            UT_FAIL("PictureMode mismatch");
            UT_LOG_DEBUG("Picture Mode : %d is not in the list of supported PictureMode ", pictureModes[j]? pictureModes[j]->value : 0);
        }
    }
    UT_LOG_DEBUG("Invoking TvTerm with no input parameters");
    status = TvTerm();
...
void test_l2_tvSettings_SetAndGetColorTemp_R_post_offset_onSource(void)
{
    ...
    tvError_t status = tvERROR_NONE;
    int rpostoffset_set = 0, rpostoffset_get = 0;
...
void test_l2_tvSettings_SetAndGetColorTempGPostOffset(void)
{
    ...
    tvError_t status = tvERROR_NONE;
    int gpostoffset_set = 0, gpostoffset_get = 0;
    tvColorTemp_t colorTemp = tvColorTemp_MAX;
    tvColorTempSourceOffset_t sourceId = MAX_OFFSET;
    int saveOnly = 0;
...
void test_l2_tvSettings_SetAndGetColorTempBPostOffset(void)
{
    ...
    tvError_t status = tvERROR_NONE;
    int bpostoffset_set = 0, bpostoffset_get = 0;
    tvColorTemp_t colorTemp = tvColorTemp_MAX;
    tvColorTempSourceOffset_t sourceId = MAX_OFFSET;
    int saveOnly = 0;
...
*
* This test case verifies if the White Balance (WB) Calibration Mode in TV settings
* can be successfully disabled and the status is correctly reflected. The test is crucial
* to ensure the correct functioning of the WB Calibration Mode toggle feature.
*
* **Test Group ID:** 02@n
* **Test Case ID:** 034@n
*
...
void test_l2_tvSettings_GetSupportedComponentColor(void)
{
    ...
    pInstance = ut_kvp_profile_getInstance();
    count = ut_kvp_getListCount(pInstance,"tvSettings/SupportedComponentColor");
    for (int i=0;i<count;i++)
    {
        flag = 0;
...
void test_l2_tvSettings_SetAndGetComponentHue(void)
{
    ...
    tvError_t status = tvERROR_NONE;
    int hue = 0, hue_get = 0;
    tvDataComponentColor_t color = tvDataColor_MAX;
    srand(time(NULL));
...
int test_l2_tvSettings_register(void)
{
    ...
    UT_add_test( pSuite, "L2_DisableAndVerifyWBCalibrationMode", test_l2_tvSettings_DisableAndVerifyWBCalibrationMode);
