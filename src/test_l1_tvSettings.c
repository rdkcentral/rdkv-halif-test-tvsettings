...
void test_l1_tvSettings_positive_GetTVSupportedVideoFormats (void)
{
    ...
    {
        tvVideoFormatsPtr[i] = &tvVideoFormats[i];
    }
    /* Step 02: Calling tvsettings GetTVSupportedVideoFormats and expectinging the API to return success */
    result = GetTVSupportedVideoFormats(tvVideoFormatsPtr, &sizeReceived);
    UT_ASSERT_EQUAL(result, tvERROR_NONE);
    ...
    for (int i = 0; i < VIDEO_FORMAT_MAX; i++)
    {
            tvVideoFormatsRetryPtr[i] = &tvVideoFormatsRetry[i];
    }
    /* Step 03: Calling tvsettings GetTVSupportedVideoFormats and expecting the API to return success */
    result = GetTVSupportedVideoFormats(tvVideoFormatsRetryPtr, &sizeReceivedRetry);
    UT_ASSERT_EQUAL(result, tvERROR_NONE);
...
void test_l1_tvSettings_negative_GetTVSupportedVideoFormats(void)
{
    ...
    if (extendedEnumsSupported == true)
    {
        ...
        result = GetTVSupportedVideoFormats(tvVideoFormatsPtr, &size);
        UT_ASSERT_EQUAL(result, tvERROR_INVALID_STATE);
    }
    ...
    result = GetTVSupportedVideoFormats(tvVideoFormatsPtr, NULL);
    UT_ASSERT_EQUAL(result, tvERROR_INVALID_PARAM);
...
void test_l1_tvSettings_positive_GetTVSupportedVideoSources (void)
{
    ...
    tvError_t result = tvERROR_NONE;
    tvVideoSrcType_t tvVideoSources[VIDEO_SOURCE_MAX]={0};
    tvVideoSrcType_t *tvVideoSourcesPtr[VIDEO_SOURCE_MAX]={0};
    tvVideoSrcType_t tvVideoSourcesRetry[VIDEO_SOURCE_MAX]={0};
    tvVideoSrcType_t *tvVideoSourcesRetryPtr[VIDEO_SOURCE_MAX]={0};
    ...
    {
        tvVideoSourcesPtr[i] = &tvVideoSources[i];
    }
    ...
    result = GetTVSupportedVideoSources(tvVideoSourcesPtr, &sizeReceived);
    UT_ASSERT_EQUAL(result, tvERROR_NONE);
    ...
    for (int i = 0; i < VIDEO_SOURCE_MAX ; i++)
    {
        tvVideoSourcesRetryPtr[i] = &tvVideoSourcesRetry[i];
    }
    /* Step 03: Calling tvsettings GetTVSupportedVideoSources and expectinging the API to return success */
    result = GetTVSupportedVideoSources(tvVideoSourcesRetryPtr, &sizeReceivedRetry);
    UT_ASSERT_EQUAL(result, tvERROR_NONE);
...
void test_l1_tvSettings_negative_GetTVSupportedVideoSources (void)
{
    ...
    tvError_t result = tvERROR_NONE ;
    tvVideoSrcType_t tvVideoSrcType[VIDEO_SOURCE_MAX]={0};
    tvVideoSrcType_t *tvVideoSrcTypePtr[VIDEO_SOURCE_MAX]={0};
    unsigned short numberOfSources = 0;
    for (int i = 0; i < VIDEO_SOURCE_MAX ; i++)
    {
        tvVideoSrcTypePtr[i] = &tvVideoSrcType[i];
    }

    if (extendedEnumsSupported == true)
    {
        /* Step 01: Calling tvsettings GetTVSupportedVideoSources and expecting the API to return tvERROR_INVALID_STATE */
        result = GetTVSupportedVideoSources(tvVideoSrcTypePtr, &numberOfSources);
        UT_ASSERT_EQUAL(result, tvERROR_INVALID_STATE);
    }
    ...
    result = GetTVSupportedVideoSources(tvVideoSrcTypePtr, NULL);
    UT_ASSERT_EQUAL(result, tvERROR_INVALID_PARAM);
...
void test_l1_tvSettings_positive_SaveBacklight (void)
{
    ...
    result = TvInit();
    UT_ASSERT_EQUAL_FATAL(result, tvERROR_NONE);
    videoSrcCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Backlight/source");
    pqCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Backlight/pqmode");

    videoFmtCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Backlight/format");
    ...
    {
        snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Backlight/source/%d", i);
        videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
        for (unsigned int j = 0; j < pqCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Backlight/pqmode/%d", j);
            pqValue = (int)UT_KVP_PROFILE_GET_UINT32(keyValue);
            for (unsigned int k = 0; k < videoFmtCount; k++)
            {
                snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Backlight/format/%d", k);
...
void test_l1_tvSettings_negative_SaveBacklight (void)
{
    ...
    videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/Backlight/source/0");
    pqValue = (int)UT_KVP_PROFILE_GET_UINT32("tvSettings/Backlight/pqmode/0");
    videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/Backlight/format/0");
    if (extendedEnumsSupported == true)
    {
    ...
    count = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Backlight/source");
    for(int i =VIDEO_SOURCE_ALL ; i < VIDEO_SOURCE_MAX; i++)
    {
        SupportAvailable = false;
        for(int j =0 ; j < count; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Backlight/source/%d", j);
            videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoSource == i)
            {
    ...
    count = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Backlight/pqmode");
    ...
    count = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Backlight/format");
    pqValue = (int)UT_KVP_PROFILE_GET_UINT32("tvSettings/Backlight/pqmode/0");
    for(int i =VIDEO_FORMAT_NONE ; i < VIDEO_FORMAT_MAX; i++)
    {
        ...
        for(int j =0 ; j < count; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Backlight/format/%d", j);
            videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoFormat == i)
            {
                SupportAvailable = true;
...
void test_l1_tvSettings_positive_GetTVSupportedDimmingModes (void)
{
    ...
    {
        tvDimmingModesPtr[i] = &tvDimmingModes[i];
        tvDimmingModesRetryPtr[i] = &tvDimmingModesRetry[i];

    }
    /* Step 02: Calling tvsettings GetTVSupportedDimmingModes and expectinging the API to return success */
    result = GetTVSupportedDimmingModes(tvDimmingModesPtr, &sizeReceived);
    UT_ASSERT_EQUAL(result, tvERROR_NONE);
    ...
    result = GetTVSupportedDimmingModes(tvDimmingModesRetryPtr, &sizeReceivedRetry);
    UT_ASSERT_EQUAL(result, tvERROR_NONE);
    UT_ASSERT_EQUAL(sizeReceived, sizeReceivedRetry);
...
void test_l1_tvSettings_negative_GetTVSupportedDimmingModes (void)
{
    ...
    if (extendedEnumsSupported == true)
    {
        ...
        result = GetTVSupportedDimmingModes(tvDimmingModesPtr, &size);
        UT_ASSERT_EQUAL(result, tvERROR_INVALID_STATE);
    }
    ...
    result = GetTVSupportedDimmingModes(tvDimmingModesPtr, NULL);
    UT_ASSERT_EQUAL(result, tvERROR_INVALID_PARAM);
...
void test_l1_tvSettings_positive_SaveTVDimmingMode (void)
{
    ...
    videoSrcCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/DimmingMode/source");
    pqCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/DimmingMode/pqmode");

    videoFmtCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/DimmingMode/format");
    dimmingModeCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/DimmingMode/index");
    for (unsigned int i = 0; i < videoSrcCount; i++)
    {
        snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/DimmingMode/source/%d", i);
        videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
        for (unsigned int j = 0; j < pqCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/DimmingMode/pqmode/%d", j);
            pqValue = (int)UT_KVP_PROFILE_GET_UINT32(keyValue);
            for (unsigned int k = 0; k < videoFmtCount; k++)
            {
                snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/DimmingMode/format/%d", k);
                videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
...
void test_l1_tvSettings_negative_SaveTVDimmingMode (void)
{
    ...
    videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/DimmingMode/source/0");
    pqValue = (int)UT_KVP_PROFILE_GET_UINT32("tvSettings/DimmingMode/pqmode/0");
    videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/DimmingMode/format/0");
    ...
    for( int i = VIDEO_SOURCE_ALL  ; i < VIDEO_SOURCE_MAX ; i++)
    {
        ...
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/DimmingMode/source/%d", j);
            videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoSource == i)
            {
                SupportAvailable = true;
    ...
    for(int i =VIDEO_FORMAT_NONE ; i < VIDEO_FORMAT_MAX; i++)
    {
        ...
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/DimmingMode/format/%d", j);
            videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoFormat == i)
            {
                SupportAvailable = true;
...
void test_l1_tvSettings_positive_SaveLocalDimmingLevel (void)
{
    ...
    videoSrcCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/LocalDimmingLevel/source");
    pqCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/LocalDimmingLevel/pqmode");

    videoFmtCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/LocalDimmingLevel/format");
    for (unsigned int i = 0; i < videoSrcCount; i++)
    {
        snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/LocalDimmingLevel/source/%d", i);
        videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
        for (unsigned int j = 0; j < pqCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/LocalDimmingLevel/pqmode/%d", j);
            pqValue = (int)UT_KVP_PROFILE_GET_UINT32(keyValue);
            for (unsigned int k = 0; k < videoFmtCount; k++)
            {
                snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/LocalDimmingLevel/format/%d", k);
                videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
...
void test_l1_tvSettings_negative_SaveLocalDimmingLevel (void)
{
    ...
    videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/LocalDimmingLevel/source/0");
    pqValue = (int)UT_KVP_PROFILE_GET_UINT32("tvSettings/LocalDimmingLevel/pqmode/0");
    videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/LocalDimmingLevel/format/0");
    dimmingLevel = (ldimStateLevel_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/TMAX/DimmingLevel/index/0");
    if (extendedEnumsSupported == true)
    {
    ...
    for(int i =VIDEO_SOURCE_ALL ; i < VIDEO_SOURCE_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoSrcCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/LocalDimmingLevel/source/%d", j);
            videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoSource == i)
            {
                SupportAvailable = true;
    ...
    for(int i =VIDEO_FORMAT_NONE ; i < VIDEO_FORMAT_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoFmtCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/LocalDimmingLevel/format/%d", j);
            videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoFormat == i)
            {
                SupportAvailable = true;
...
void test_l1_tvSettings_positive_SaveBrightness (void)
{
    ...
    videoSrcCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Brightness/source");
    pqCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Brightness/pqmode");

    videoFmtCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Brightness/format");
    for (unsigned int i = 0; i < videoSrcCount; i++)
    {
        snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Brightness/source/%d", i);
        videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
        for (unsigned int j = 0; j < pqCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Brightness/pqmode/%d", j);
            pqValue = (int)UT_KVP_PROFILE_GET_UINT32(keyValue);
            for (unsigned int k = 0; k < videoFmtCount; k++)
            {
                snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Brightness/format/%d", k);
                videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
...
void test_l1_tvSettings_negative_SaveBrightness (void)
{
    ...
    videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/Brightness/source/0");
    pqValue = (int)UT_KVP_PROFILE_GET_UINT32("tvSettings/Brightness/pqmode/0");
    videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/Brightness/format/0");
    if (extendedEnumsSupported == true)
    {
    ...
    count = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Brightness/source");
    for(int i =VIDEO_SOURCE_ALL ; i < VIDEO_SOURCE_MAX; i++)
    {
        SupportAvailable = false;
        for(int j =0 ; j < count; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Brightness/source/%d", j);
            videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoSource == i)
            {
                SupportAvailable = true;
    ...
    videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/Brightness/source/0");
    count = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Brightness/pqmode");
    ...
    pqValue = (int)UT_KVP_PROFILE_GET_UINT32("tvSettings/Brightness/pqmode/0");
    count = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Brightness/format");
    for(int i =VIDEO_FORMAT_NONE ; i < VIDEO_FORMAT_MAX; i++)
    {
        SupportAvailable = false;
        for(int j =0 ; j < count; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Brightness/format/%d", j);
            videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoFormat == i)
            {
                SupportAvailable = true;
...
void test_l1_tvSettings_positive_SaveContrast (void)
{
    ...
    videoSrcCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Contrast/source");
    pqCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Contrast/pqmode");

    videoFmtCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Contrast/format");
    for (unsigned int i = 0; i < videoSrcCount; i++)
    {
        snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Contrast/source/%d", i);
        videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
        for (unsigned int j = 0; j < pqCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Contrast/pqmode/%d", j);
            pqValue = (int)UT_KVP_PROFILE_GET_UINT32(keyValue);
            for (unsigned int k = 0; k < videoFmtCount; k++)
            {
                snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Contrast/format/%d", k);
                videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
...
void test_l1_tvSettings_negative_SaveContrast (void)
{
    ...
    videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/Contrast/source/0");
    pqValue = (int)UT_KVP_PROFILE_GET_UINT32("tvSettings/Contrast/pqmode/0");
    videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/Contrast/format/0");
    if (extendedEnumsSupported == true)
    {
    ...
    for(int i =VIDEO_SOURCE_ALL ; i < VIDEO_SOURCE_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoSrcCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Contrast/source/%d", j);
            videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoSource == i)
            {
                SupportAvailable = true;
    ...
    for(int i =VIDEO_FORMAT_NONE ; i < VIDEO_FORMAT_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoFmtCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Contrast/format/%d", j);
            videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoFormat == i)
            {
                SupportAvailable = true;
...
void test_l1_tvSettings_positive_SaveSharpness (void)
{
    ...
    videoSrcCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Sharpness/source");
    pqCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Sharpness/pqmode");

    videoFmtCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Sharpness/format");
    for (unsigned int i = 0; i < videoSrcCount; i++)
    {
        snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Sharpness/source/%d", i);
        videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
        for (unsigned int j = 0; j < pqCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Sharpness/pqmode/%d", j);
            pqValue = (int)UT_KVP_PROFILE_GET_UINT32(keyValue);
            for (unsigned int k = 0; k < videoFmtCount; k++)
            {
                snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Sharpness/source/%d", k);
                videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
...
void test_l1_tvSettings_negative_SaveSharpness (void)
{
    ...
    videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/Sharpness/source/0");
    pqValue = (int)UT_KVP_PROFILE_GET_UINT32("tvSettings/Sharpness/pqmode/0");
    videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/Sharpness/format/0");
    if (extendedEnumsSupported == true)
    {
    ...
    for(int i =VIDEO_SOURCE_ALL ; i < VIDEO_SOURCE_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoSrcCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Sharpness/source/%d", j);
            videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoSource == i)
            {
                SupportAvailable = true;
    ...
    for(int i =VIDEO_FORMAT_NONE ; i < VIDEO_FORMAT_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoFmtCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Sharpness/format/%d", j);
            videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoFormat == i)
            {
                SupportAvailable = true;
...
void test_l1_tvSettings_positive_SaveSaturation (void)
{
    ...
    videoSrcCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Saturation/source");
    pqCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Saturation/pqmode");

    videoFmtCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Saturation/format");
    for (unsigned int i = 0; i < videoSrcCount; i++)
    {
        snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Saturation/source/%d", i);
        videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
        for (unsigned int j = 0; j < pqCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Saturation/pqmode/%d", j);
            pqValue = (int)UT_KVP_PROFILE_GET_UINT32(keyValue);
            for (unsigned int k = 0; k < videoFmtCount; k++)
            {
                snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Saturation/format/%d", k);
                videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
...
void test_l1_tvSettings_negative_SaveSaturation (void)
{
    ...
    videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/Saturation/source/0");
    pqValue = (int)UT_KVP_PROFILE_GET_UINT32("tvSettings/Saturation/pqmode/0");
    videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/Saturation/format/0");
    if (extendedEnumsSupported == true)
    {
    ...
    for(int i =VIDEO_SOURCE_ALL ; i < VIDEO_SOURCE_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoSrcCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Saturation/format/%d", j);
            videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoSource == i)
            {
                SupportAvailable = true;
...
void test_l1_tvSettings_positive_SaveHue (void)
{
    ...
    videoSrcCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Hue/source");
    pqCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Hue/pqmode");

    videoFmtCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/Hue/format");
    for (unsigned int i = 0; i < videoSrcCount; i++)
    {
        snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Hue/source/%d", i);
        videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
        for (unsigned int j = 0; j < pqCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Hue/pqmode/%d", j);
            pqValue = (int)UT_KVP_PROFILE_GET_UINT32(keyValue);
            for (unsigned int k = 0; k < videoFmtCount; k++)
            {
                snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Hue/format/%d", k);
                videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
...
void test_l1_tvSettings_negative_SaveHue (void)
{
    ...
    videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/Hue/source/0");
    pqValue = (int)UT_KVP_PROFILE_GET_UINT32("tvSettings/Hue/pqmode/0");
    videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/Hue/format/0");
    if (extendedEnumsSupported == true)
    {
    ...
    for(int i =VIDEO_SOURCE_ALL ; i < VIDEO_SOURCE_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoSrcCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Hue/source/%d", j);
            videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoSource == i)
            {
                SupportAvailable = true;
    ...
    for(int i =VIDEO_FORMAT_NONE ; i < VIDEO_FORMAT_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoFmtCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/Hue/format/%d", j);
            videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoFormat == i)
            {
                SupportAvailable = true;
...
void test_l1_tvSettings_positive_SaveColorTemperature (void)
{
    ...
    videoSrcCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/ColorTemperature/source");
    pqCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/ColorTemperature/pqmode");

    videoFmtCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/ColorTemperature/format");
    colorTempCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/ColorTemperature/index");
    for (unsigned int i = 0; i < videoSrcCount; i++)
    {
        snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/ColorTemperature/source/%d", i);
        videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
        for (unsigned int j = 0; j < pqCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/ColorTemperature/pqmode/%d", j);
            pqValue = (int)UT_KVP_PROFILE_GET_UINT32(keyValue);
            for (unsigned int k = 0; k < videoFmtCount; k++)
            {
                snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/ColorTemperature/format/%d", k);
                videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
...
void test_l1_tvSettings_negative_SaveColorTemperature (void)
{
    ...
    videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/ColorTemperature/source/0");
    pqValue = (int)UT_KVP_PROFILE_GET_UINT32("tvSettings/ColorTemperature/pqmode/0");
    videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/ColorTemperature/format/0");
    colorTempValue = (tvColorTemp_t) UT_KVP_PROFILE_GET_UINT32("tvSettings/ColorTemperature/index/0");
    if (extendedEnumsSupported == true)
    {
    ...
    for(int i =VIDEO_SOURCE_ALL ; i < VIDEO_SOURCE_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoSrcCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/ColorTemperature/source/%d", j);
            videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoSource == i)
            {
                SupportAvailable = true;
    ...
    for(int i =VIDEO_FORMAT_NONE ; i < VIDEO_FORMAT_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoFmtCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/ColorTemperature/format/%d", j);
            videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoFormat == i)
            {
                SupportAvailable = true;
...
void test_l1_tvSettings_positive_SaveAspectRatio (void)
{
    ...
    videoSrcCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/AspectRatio/source");
    pqCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/AspectRatio/pqmode");

    videoFmtCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/AspectRatio/format");
    aspectRatioCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/AspectRatio/index");
    for (unsigned int i = 0; i < videoSrcCount; i++)
    {
        snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/AspectRatio/source/%d", i);
        videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
        for (unsigned int j = 0; j < pqCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/AspectRatio/pqmode/%d", j);
            pqValue = (int)UT_KVP_PROFILE_GET_UINT32(keyValue);
            for (unsigned int k = 0; k < videoFmtCount; k++)
            {
                snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/AspectRatio/format/%d", k);
                videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
...
void test_l1_tvSettings_negative_SaveAspectRatio (void)
{
    ...
    videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/AspectRatio/source/0");
    pqValue = (int)UT_KVP_PROFILE_GET_UINT32("tvSettings/AspectRatio/pqmode/0");
    videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/AspectRatio/format/0");
    aspectRatioValue = UT_KVP_PROFILE_GET_UINT32("tvSettings/AspectRatio/index/0");
    if (extendedEnumsSupported == true)
    {
    ...
    for(int i =VIDEO_SOURCE_ALL ; i < VIDEO_SOURCE_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoSrcCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/AspectRatio/source/%d", j);
            videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoSource == i)
            {
                SupportAvailable = true;
    ...
    for(int i =VIDEO_FORMAT_NONE ; i < VIDEO_FORMAT_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoFmtCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/AspectRatio/format/%d", j);
            videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoFormat == i)
            {
                SupportAvailable = true;
...
void test_l1_tvSettings_positive_SaveLowLatencyState (void)
{
    ...
    videoSrcCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/LowLatencyState/source");
    pqCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/LowLatencyState/pqmode");

    videoFmtCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/LowLatencyState/format");
    for (unsigned int i = 0; i < videoSrcCount; i++)
    {
        snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/LowLatencyState/source/%d", i);
        videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
        for (unsigned int j = 0; j < pqCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/LowLatencyState/pqmode/%d", j);
            pqValue = (int)UT_KVP_PROFILE_GET_UINT32(keyValue);
            for (unsigned int k = 0; k < videoFmtCount; k++)
            {
                snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/LowLatencyState/format/%d", k);
                videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
...
void test_l1_tvSettings_negative_SaveLowLatencyState (void)
{
    ...
    videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/LowLatencyState/source/0");
    pqValue = (int)UT_KVP_PROFILE_GET_UINT32("tvSettings/LowLatencyState/pqmode/0");
    videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/LowLatencyState/format/0");
    if (extendedEnumsSupported == true)
    {
    ...
    for(int i =VIDEO_SOURCE_ALL ; i < VIDEO_SOURCE_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoSrcCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/LowLatencyState/source/%d", j);
            videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoSource == i)
            {
                SupportAvailable = true;
    ...
    for(int i =VIDEO_FORMAT_NONE ; i < VIDEO_FORMAT_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoFmtCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/LowLatencyState/format/%d", j);
            videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoFormat == i)
            {
                SupportAvailable = true;
...
void test_l1_tvSettings_positive_GetTVSupportedDolbyVisionModes (void)
{
    ...
    {
        tvDolbyModesPtr[i] = &tvDolbyModes[i];
        tvDolbyModesRetryPtr[i] = &tvDolbyModesRetry[i];
    }

    /* Step 02: Calling tvsettings GetTVSupportedDolbyVisionModes and expectinging the API to return success */
    result = GetTVSupportedDolbyVisionModes(tvDolbyModesPtr, &sizeReceived);
    UT_ASSERT_EQUAL(result, tvERROR_NONE);
    ...
    for (unsigned int i = 0; i < dolbyVisonModeCount; i++)
    {
        ...
        for (unsigned short j = 0; j < sizeReceived; j++)
        {
            ...
            if (dolbyVisionModevalue == tvDolbyModes[j])
            {
    ...
    result = GetTVSupportedDolbyVisionModes(tvDolbyModesRetryPtr, &sizeReceivedRetry);
    UT_ASSERT_EQUAL(result, tvERROR_NONE);
    UT_ASSERT_EQUAL(sizeReceived, sizeReceivedRetry);
...
void test_l1_tvSettings_negative_GetTVSupportedDolbyVisionModes (void)
{
    ...
    if (extendedEnumsSupported == true)
    {
        ...
        result = GetTVSupportedDolbyVisionModes(tvDolbyModesPtr, &size);
        UT_ASSERT_EQUAL(result, tvERROR_INVALID_STATE);
    }
    ...
    result = GetTVSupportedDolbyVisionModes(tvDolbyModesPtr, NULL);
    UT_ASSERT_EQUAL(result, tvERROR_INVALID_PARAM);
...
void test_l1_tvSettings_positive_SaveTVDolbyVisionMode (void)
{
    ...
    videoSrcCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/DolbyVisionMode/source");
    pqCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/DolbyVisionMode/pqmode");

    videoFmtCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/DolbyVisionMode/format");
    dolbyVisonModeCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/DolbyVisionMode/index");
    for (unsigned int i = 0; i < videoSrcCount; i++)
    {
        snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/DolbyVisionMode/source/%d", i);
        videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
        for (unsigned int j = 0; j < pqCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/DolbyVisionMode/pqmode/%d", j);
            pqValue = (int)UT_KVP_PROFILE_GET_UINT32(keyValue);
            for (unsigned int k = 0; k < videoFmtCount; k++)
            {
                snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/DolbyVisionMode/format/%d", k);
                videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
...
void test_l1_tvSettings_negative_SaveTVDolbyVisionMode (void)
{
    ...
    videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/DolbyVisionMode/source/0");
    pqValue = (int)UT_KVP_PROFILE_GET_UINT32("tvSettings/DolbyVisionMode/pqmode/0");
    videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/DolbyVisionMode/format/0");
    dolbyVisionModevalue = (tvDolbyMode_t) UT_KVP_PROFILE_GET_UINT32("tvSettings/DolbyVisionMode/index/0");
    if (extendedEnumsSupported == true)
    {
    ...
    for( int i = VIDEO_SOURCE_ALL  ; i < VIDEO_SOURCE_MAX ; i++)
    {
        ...
        for(int j =0 ; j < videoSrcCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/DolbyVisionMode/source/%d", j);
            videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoSource == i)
            {
                SupportAvailable = true;
    ...
    for(int i =VIDEO_FORMAT_NONE ; i < VIDEO_FORMAT_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoFmtCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/DolbyVisionMode/format/%d", j);
            videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoFormat == i)
            {
                SupportAvailable = true;
...
void test_l1_tvSettings_positive_GetTVSupportedPictureModes (void)
{
    ...
    {
        tvPicModesPtr[i] = &tvPicModes[i];
        tvPicModesRetryPtr[i] = &tvPicModesRetry[i];
    }
    /* Step 02: Calling tvsettings GetTVSupportedPictureModes and expectinging the API to return success */
    pqCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/PictureMode/index");
    result = GetTVSupportedPictureModes(tvPicModesPtr, &sizeReceived);
    UT_ASSERT_EQUAL(result, tvERROR_NONE);
    ...
    result = GetTVSupportedPictureModes(tvPicModesRetryPtr, &sizeReceivedRetry);
    UT_ASSERT_EQUAL(result, tvERROR_NONE);
    UT_ASSERT_EQUAL(sizeReceived, sizeReceivedRetry);
    for (unsigned short i = 0; i < sizeReceivedRetry; i++)
    {
...
void test_l1_tvSettings_negative_GetTVSupportedPictureModes (void)
{
    ...
    result = GetTVSupportedPictureModes(pic_modesPtr, NULL);
    UT_ASSERT_EQUAL(result, tvERROR_INVALID_PARAM);
...
void test_l1_tvSettings_positive_SaveSourcePictureMode (void)
{
    ...
    videoSrcCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/PictureMode/source");
    pqCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/PictureMode/index");

    videoFmtCount = UT_KVP_PROFILE_GET_LIST_COUNT("tvSettings/PictureMode/format");
    for (unsigned int i = 0; i < videoSrcCount; i++)
    {
        snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/PictureMode/source/%d", i);
        videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
        for (unsigned int j = 0; j < pqCount; j++)
        {
            ...
            for (unsigned int k = 0; k < videoFmtCount; k++)
            {
                snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/PictureMode/format/%d", k);
                videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
...
void test_l1_tvSettings_negative_SaveSourcePictureMode (void)
{
    ...
    videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/PictureMode/source/0");
    pqValue = (int)UT_KVP_PROFILE_GET_UINT32("tvSettings/PictureMode/pqmode/0");
    videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32("tvSettings/PictureMode/format/0");
    if (extendedEnumsSupported == true)
    {
    ...
    for( int i = VIDEO_SOURCE_ALL  ; i < VIDEO_SOURCE_MAX ; i++)
    {
        ...
        for(int j =0 ; j < videoSrcCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/PictureMode/source/%d", j);
            videoSource = (tvVideoSrcType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoSource == i)
            {
                SupportAvailable = true;
    ...
    for(int i =VIDEO_FORMAT_NONE ; i < VIDEO_FORMAT_MAX; i++)
    {
        ...
        for(int j =0 ; j < videoFmtCount; j++)
        {
            snprintf(keyValue, UT_KVP_MAX_ELEMENT_SIZE, "tvSettings/PictureMode/format/%d", j);
            videoFormat = (tvVideoFormatType_t)UT_KVP_PROFILE_GET_UINT32(keyValue);
            if(videoFormat == i)
            {
                SupportAvailable = true;
...
int test_l1_tvSettings_register ( void )
{
    ...
    UT_add_test( pSuite, "GetTVSupportedVideoSources_pos" ,test_l1_tvSettings_positive_GetTVSupportedVideoSources );
    UT_add_test( pSuite, "GetTVSupportedVideoSources_neg" ,test_l1_tvSettings_negative_GetTVSupportedVideoSources );
    ...
    UT_add_test( pSuite, "GetSupportedBacklightModes_pos" ,test_l1_tvSettings_positive_GetSupportedBacklightModes );
    UT_add_test( pSuite, "GetSupportedBacklightModes_neg" ,test_l1_tvSettings_negative_GetSupportedBacklightModes );
    UT_add_test( pSuite, "GetCurrentBacklightMode_pos" ,test_l1_tvSettings_positive_GetCurrentBacklightMode );
    UT_add_test( pSuite, "GetCurrentBacklightMode_neg" ,test_l1_tvSettings_negative_GetCurrentBacklightMode );
    UT_add_test( pSuite, "SetCurrentBacklightMode_pos" ,test_l1_tvSettings_positive_SetCurrentBacklightMode );
    UT_add_test( pSuite, "SetCurrentBacklightMode_neg" ,test_l1_tvSettings_negative_SetCurrentBacklightMode );
    UT_add_test( pSuite, "GetTVSupportedDimmingModes_pos" ,test_l1_tvSettings_positive_GetTVSupportedDimmingModes );
    UT_add_test( pSuite, "GetTVSupportedDimmingModes_neg" ,test_l1_tvSettings_negative_GetTVSupportedDimmingModes );
    ...
    UT_add_test( pSuite, "SetLocalDimmingLevel_pos" ,test_l1_tvSettings_positive_SetLocalDimmingLevel );
    UT_add_test( pSuite, "SetLocalDimmingLevel_neg" ,test_l1_tvSettings_negative_SetLocalDimmingLevel );
    UT_add_test( pSuite, "GetLocalDimmingLevel_pos" ,test_l1_tvSettings_positive_GetLocalDimmingLevel );
    UT_add_test( pSuite, "GetLocalDimmingLevel_neg" ,test_l1_tvSettings_negative_GetLocalDimmingLevel );
    UT_add_test( pSuite, "SaveLocalDimmingLevel_pos" ,test_l1_tvSettings_positive_SaveLocalDimmingLevel );
    UT_add_test( pSuite, "SaveLocalDimmingLevel_neg" ,test_l1_tvSettings_negative_SaveLocalDimmingLevel );
    ...
    UT_add_test( pSuite, "SaveSaturation_nega" ,test_l1_tvSettings_negative_SaveSaturation );
    UT_add_test( pSuite, "GetNumberOfDimmingZones_pos" ,test_l1_tvSettings_positive_GetNumberOfDimmingZones );
    UT_add_test( pSuite, "GetNumberOfDimmingZones_neg" ,test_l1_tvSettings_negative_GetNumberOfDimmingZones );
    ...
    UT_add_test( pSuite, "SetColorTemperature_pos" ,test_l1_tvSettings_positive_SetColorTemperature );
    UT_add_test( pSuite, "SetColorTemperature_neg" ,test_l1_tvSettings_negative_SetColorTemperature );
    UT_add_test( pSuite, "GetColorTemperature_pos" ,test_l1_tvSettings_positive_GetColorTemperature );
    UT_add_test( pSuite, "GetColorTemperature_neg" ,test_l1_tvSettings_negative_GetColorTemperature );
    UT_add_test( pSuite, "SaveColorTemperature_pos" ,test_l1_tvSettings_positive_SaveColorTemperature );
    UT_add_test( pSuite, "SaveColorTemperature_neg" ,test_l1_tvSettings_negative_SaveColorTemperature );
    ...
    UT_add_test( pSuite, "SetLowLatencyState_pos" ,test_l1_tvSettings_positive_SetLowLatencyState );
    UT_add_test( pSuite, "SetLowLatencyState_neg" ,test_l1_tvSettings_negative_SetLowLatencyState );
    UT_add_test( pSuite, "GetLowLatencyState_pos" ,test_l1_tvSettings_positive_GetLowLatencyState );
    UT_add_test( pSuite, "GetLowLatencyState_neg" ,test_l1_tvSettings_negative_GetLowLatencyState );
    UT_add_test( pSuite, "SaveLowLatencyState_pos" ,test_l1_tvSettings_positive_SaveLowLatencyState );
    UT_add_test( pSuite, "SaveLowLatencyState_neg" ,test_l1_tvSettings_negative_SaveLowLatencyState );
    UT_add_test( pSuite, "SetDynamicContrast_pos" ,test_l1_tvSettings_positive_SetDynamicContrast );
    UT_add_test( pSuite, "SetDynamicContrast_neg" ,test_l1_tvSettings_negative_SetDynamicContrast );
    UT_add_test( pSuite, "GetDynamicContrast_pos" ,test_l1_tvSettings_positive_GetDynamicContrast );
    UT_add_test( pSuite, "GetDynamicContrast_neg" ,test_l1_tvSettings_negative_GetDynamicContrast );
    UT_add_test( pSuite, "SetDynamicGamma_pos" ,test_l1_tvSettings_positive_SetDynamicGamma );
    UT_add_test( pSuite, "SetDynamicGamma_neg" ,test_l1_tvSettings_negative_SetDynamicGamma );
    UT_add_test( pSuite, "GetDynamicGamma_pos" ,test_l1_tvSettings_positive_GetDynamicGamma );
    UT_add_test( pSuite, "GetDynamicGamma_neg" ,test_l1_tvSettings_negative_GetDynamicGamma );
    UT_add_test( pSuite, "GetTVSupportedDolbyVisionModes_pos" ,test_l1_tvSettings_positive_GetTVSupportedDolbyVisionModes );
    UT_add_test( pSuite, "GetTVSupportedDolbyVisionModes_neg" ,test_l1_tvSettings_negative_GetTVSupportedDolbyVisionModes );
    ...
    UT_add_test( pSuite, "GetTVSupportedPictureModes_pos" ,test_l1_tvSettings_positive_GetTVSupportedPictureModes );
    UT_add_test( pSuite, "GetTVSupportedPictureModes_neg" ,test_l1_tvSettings_negative_GetTVSupportedPictureModes );
    ...
    UT_add_test( pSuite, "SetColorTemp_Rgain_onSource_pos" ,test_l1_tvSettings_positive_SetColorTemp_Rgain_onSource );
    UT_add_test( pSuite, "SetColorTemp_Rgain_onSource_neg" ,test_l1_tvSettings_negative_SetColorTemp_Rgain_onSource );
    UT_add_test( pSuite, "GetColorTemp_Rgain_onSource_pos" ,test_l1_tvSettings_positive_GetColorTemp_Rgain_onSource );
    UT_add_test( pSuite, "GetColorTemp_Rgain_onSource_neg" ,test_l1_tvSettings_negative_GetColorTemp_Rgain_onSource );
    UT_add_test( pSuite, "SetColorTemp_Ggain_onSource_pos" ,test_l1_tvSettings_positive_SetColorTemp_Ggain_onSource );
    UT_add_test( pSuite, "SetColorTemp_Ggain_onSource_neg" ,test_l1_tvSettings_negative_SetColorTemp_Ggain_onSource );
    UT_add_test( pSuite, "GetColorTemp_Ggain_onSource_pos" ,test_l1_tvSettings_positive_GetColorTemp_Ggain_onSource );
    UT_add_test( pSuite, "GetColorTemp_Ggain_onSource_neg" ,test_l1_tvSettings_negative_GetColorTemp_Ggain_onSource );
    UT_add_test( pSuite, "SetColorTemp_Bgain_onSource_pos" ,test_l1_tvSettings_positive_SetColorTemp_Bgain_onSource );
    UT_add_test( pSuite, "SetColorTemp_Bgain_onSource_neg" ,test_l1_tvSettings_negative_SetColorTemp_Bgain_onSource );
    UT_add_test( pSuite, "GetColorTemp_Bgain_onSource_pos" ,test_l1_tvSettings_positive_GetColorTemp_Bgain_onSource );
    UT_add_test( pSuite, "GetColorTemp_Bgain_onSource_neg" ,test_l1_tvSettings_negative_GetColorTemp_Bgain_onSource );
    UT_add_test( pSuite, "SetColorTemp_R_post_offset_onSource_pos" ,test_l1_tvSettings_positive_SetColorTemp_R_post_offset_onSource );
    UT_add_test( pSuite, "SetColorTemp_R_post_offset_onSource_neg" ,test_l1_tvSettings_negative_SetColorTemp_R_post_offset_onSource );
    UT_add_test( pSuite, "GetColorTemp_R_post_offset_onSource_pos" ,test_l1_tvSettings_positive_GetColorTemp_R_post_offset_onSource );
    UT_add_test( pSuite, "GetColorTemp_R_post_offset_onSource_neg" ,test_l1_tvSettings_negative_GetColorTemp_R_post_offset_onSource );
    UT_add_test( pSuite, "SetColorTemp_G_post_offset_onSource_pos" ,test_l1_tvSettings_positive_SetColorTemp_G_post_offset_onSource );
    UT_add_test( pSuite, "SetColorTemp_G_post_offset_onSource_neg" ,test_l1_tvSettings_negative_SetColorTemp_G_post_offset_onSource );
    UT_add_test( pSuite, "GetColorTemp_G_post_offset_onSource_pos" ,test_l1_tvSettings_positive_GetColorTemp_G_post_offset_onSource );
    UT_add_test( pSuite, "GetColorTemp_G_post_offset_onSource_neg" ,test_l1_tvSettings_negative_GetColorTemp_G_post_offset_onSource );
    UT_add_test( pSuite, "SetColorTemp_B_post_offset_onSource_pos" ,test_l1_tvSettings_positive_SetColorTemp_B_post_offset_onSource );
    UT_add_test( pSuite, "SetColorTemp_B_post_offset_onSource_neg" ,test_l1_tvSettings_negative_SetColorTemp_B_post_offset_onSource );
    UT_add_test( pSuite, "GetColorTemp_B_post_offset_onSource_pos" ,test_l1_tvSettings_positive_GetColorTemp_B_post_offset_onSource );
    UT_add_test( pSuite, "GetColorTemp_B_post_offset_onSource_neg" ,test_l1_tvSettings_negative_GetColorTemp_B_post_offset_onSource );
    ...
    UT_add_test( pSuite, "GetSupportedComponentColor_pos" ,test_l1_tvSettings_positive_GetSupportedComponentColor );
    UT_add_test( pSuite, "GetSupportedComponentColor_neg" ,test_l1_tvSettings_negative_GetSupportedComponentColor );
    UT_add_test( pSuite, "SetCurrentComponentSaturation_pos" ,test_l1_tvSettings_positive_SetCurrentComponentSaturation );
    UT_add_test( pSuite, "SetCurrentComponentSaturation_neg" ,test_l1_tvSettings_negative_SetCurrentComponentSaturation );
    UT_add_test( pSuite, "GetCurrentComponentSaturation_pos" ,test_l1_tvSettings_positive_GetCurrentComponentSaturation );
    UT_add_test( pSuite, "GetCurrentComponentSaturation_neg" ,test_l1_tvSettings_negative_GetCurrentComponentSaturation );
    ...
    UT_add_test( pSuite, "EnableLDIMPixelCompensation_pos" ,test_l1_tvSettings_positive_EnableLDIMPixelCompensation );
    UT_add_test( pSuite, "EnableLDIMPixelCompensation_neg" ,test_l1_tvSettings_negative_EnableLDIMPixelCompensation );
    ...
    UT_add_test( pSuite, "GetLdimZoneShortCircuitStatus_pos" ,test_l1_tvSettings_positive_GetLdimZoneShortCircuitStatus );
    UT_add_test( pSuite, "GetLdimZoneShortCircuitStatus_neg" ,test_l1_tvSettings_negative_GetLdimZoneShortCircuitStatus );
    ...
    UT_add_test( pSuite, "RegisterVideoResolutionChangeCB_pos" ,test_l1_tvSettings_positive_RegisterVideoResolutionChangeCB );
    UT_add_test( pSuite, "RegisterVideoResolutionChangeCB_neg" ,test_l1_tvSettings_negative_RegisterVideoResolutionChangeCB );
    UT_add_test( pSuite, "RegisterVideoFrameRateChangeCB_pos" ,test_l1_tvSettings_positive_RegisterVideoFrameRateChangeCB );
    UT_add_test( pSuite, "RegisterVideoFrameRateChangeCB_neg" ,test_l1_tvSettings_negative_RegisterVideoFrameRateChangeCB );
