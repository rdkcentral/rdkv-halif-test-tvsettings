...
class tvSettings_test28_BpostOffset(utHelperClass):

    testName = "test28_BpostOffset"
    testSetupPath = os.path.join(dir_path, "tvSettings_L3_testSetup.yml")
    moduleName = "tvSettings"
    rackDevice = "dut"
    bpostValues = [-1024, -512, 0, 512, 1023]  # BpostValue range
    saveSetFlags = [0, 1]  # Save only or set only flags

    def __init__(self):
        """
        Initializes the test28 BpostOffset test.
    ...
    def testVerifyBpostOffset(self, colorTemperature, bpostValue, saveSetFlag, manual=False):
        ...
        Returns:
            bool : returns the status of Color Temperature and BpostOffset
        """
        if manual:
    ...
    def testFunction(self):
        """This function tests the BpostOffset settings.
            ...
            for colorTemperature in colorTemperatureValues:
                for bpostValue in self.bpostValues:
                    for saveSetFlag in self.saveSetFlags:
                        self.log.stepStart(f'Color Temperature:{colorTemperature} BpostOffset:{bpostValue} Save/Set Flag:{saveSetFlag} Stream:{stream}')

                        # Set the Color Temperature and BpostOffset
                        self.testtvSettings.setBpostOffsetValue(colorTemperature, bpostValue, saveSetFlag)

                        result = self.testVerifyBpostOffset(colorTemperature, bpostValue, saveSetFlag, True)

                        self.log.stepResult(result, f'Color Temperature:{colorTemperature} BpostOffset:{bpostValue} Save/Set Flag:{saveSetFlag} Stream:{stream}')
...
if __name__ == '__main__':
    test = tvSettings_test28_BpostOffset()
    test.run(False)
