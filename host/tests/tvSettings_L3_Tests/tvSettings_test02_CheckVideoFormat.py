...
class tvSettings_test02_CheckVideoFormat(utHelperClass):

    testName = "test02_CheckVideoFormat"
    ...
    def __init__(self):
        """
        Initializes the test02 VideoFormat test.
    ...
    def testDownloadAssets(self, stream_url):
        ...
        # Download test artifacts to device (if needed, this part remains unchanged)
    ...
    def testFunction(self):
        """This function tests the VideoFormat Levels
        ...
        for format, stream_url in zip(videoFormats, self.testSetup.get("assets").get("device").get(self.testName).get("streams")):
            ...
            self.log.stepStart(f'videoFormat Level:{format} Stream:{stream_full_path}')
...
if __name__ == '__main__':
    test = tvSettings_test02_CheckVideoFormat()
    test.run(False)
