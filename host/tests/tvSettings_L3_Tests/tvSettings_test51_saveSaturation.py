...
class tvSettings_test51_SaveSaturation(utHelperClass):

    testName = "test51_SaveSaturation"
            ...
            for pictureMode in self.testtvSettings.getPictureModeInfo():
                for videoFormat in self.testtvSettings.getVideoFormatInfo():
                    for saturation in self.saturationValues:
                        self.log.stepStart(f'Set Saturation Value: {saturation}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')

                        # Set Picture Mode, Video Format, and Saturation Value
                        self.testtvSettings.setSaturation(pictureMode, videoFormat, saturation)

                        result = self.testVerifySaturationValue(pictureMode, videoFormat, saturation, True)

                        self.log.stepResult(result, f'Saturation Value: {saturation}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')
...
if __name__ == '__main__':
    test = tvSettings_test51_SaveSaturation()
    test.run(False)
