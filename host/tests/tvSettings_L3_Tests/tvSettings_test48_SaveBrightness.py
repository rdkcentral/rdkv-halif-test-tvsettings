...
class tvSettings_test48_SaveBrightness(utHelperClass):

    testName = "test48_SaveBrightness"
            ...
            for pictureMode in self.testtvSettings.getPictureModeInfo():
                for videoFormat in self.testtvSettings.getVideoFormatInfo():
                    for brightness in self.brightnessValues:
                        self.log.stepStart(f'Set Brightness Value: {brightness}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')

                        # Set Picture Mode, Video Format, and Brightness Value
                        self.testtvSettings.setBrightness(pictureMode, videoFormat, brightness)

                        result = self.testVerifyBrightnessValue(pictureMode, videoFormat, brightness, True)

                        self.log.stepResult(result, f'Brightness Value: {brightness}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')
...
if __name__ == '__main__':
    test = tvSettings_test48_SaveBrightness()
    test.run(False)
