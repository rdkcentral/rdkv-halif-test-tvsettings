...
class tvSettings_test57_SavePictureMode(utHelperClass):

    testName = "test57_SavePictureMode"
    ...
    def __init__(self):
        ...
        Args:
            None.
        """
        super().__init__(self.testName, '57')
    ...
    def testVerifyPictureMode(self, videoFormat, pictureMode, manual=False):
        ...
        Returns:
            bool: returns the status of Picture Mode value.
        """
        if manual:
    ...
    def testFunction(self):
        """This function tests saving Picture Mode values with all combinations of video format and picture mode.
        ...
        self.log.testStart(self.testName, '57')
        ...
        for stream in self.testStreams:
            ...
            for videoFormat in self.testtvSettings.getVideoFormatInfo():
                for pictureMode in self.testtvSettings.getPictureModeInfo():
                    self.log.stepStart(f'Set Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')

                    # Set Video Format and Picture Mode
                    self.testtvSettings.setPictureMode(videoFormat, pictureMode)

                    result = self.testVerifyPictureMode(videoFormat, pictureMode, True)

                    self.log.stepResult(result, f'Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')
...
if __name__ == '__main__':
    test = tvSettings_test57_SavePictureMode()
    test.run(False)
