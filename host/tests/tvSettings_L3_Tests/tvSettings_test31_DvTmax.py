...
class tvSettings_test31_DVTmaxValue(utHelperClass):

    testName = "test31_DVTmaxValue"
    ...
    def testVerifyDVTmaxValue(self, dvtMaxValue, manual=False):
        """
        Verifies whether the DVT max value is set or not.

        Args:
            dvtMaxValue (int): DVT max value
            manual (bool, optional): Manual verification (True: manual, False: other verification methods).
                                     Defaults to other verification methods.

        Returns:
            bool: returns the status of DVT max value.
        """
        if manual:
    ...
    def testFunction(self):
        """This function tests the DVTmaxValue.
...
if __name__ == '__main__':
    test = tvSettings_test31_DVTmaxValue()
    test.run(False)
