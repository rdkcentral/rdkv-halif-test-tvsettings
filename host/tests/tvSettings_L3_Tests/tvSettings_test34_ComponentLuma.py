...
class tvSettings_test32_ComponentLuma(utHelperClass):

    testName = "test32_ComponentLuma"
    testSetupPath = os.path.join(dir_path, "tvSettings_L3_testSetup.yml")
    moduleName = "tvSettings"
    lumaValues = list(range(0, 101))  # Luma values from 0 to 100
    ...
    def testVerifyComponentLuma(self, lumaValue, manual=False):
        """
        Verifies whether the Component Luma is set or not.

        Args:
            lumaValue (int): luma value
            manual (bool, optional): Manual verification (True: manual, False: other verification methods).
                                     Defaults to other verification methods.

        Returns:
            bool: returns the status of component luma.
        """
        if manual:
    ...
    def testFunction(self):
        """This function tests the Component Luma.
