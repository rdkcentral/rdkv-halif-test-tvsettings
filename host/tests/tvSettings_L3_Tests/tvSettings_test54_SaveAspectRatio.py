...
class tvSettings_test54_SaveAspectRatio(utHelperClass):

    testName = "test54_SaveAspectRatio"
    ...
    def testVerifyAspectRatio(self, pictureMode, videoFormat, aspectRatio, manual=False):
        ...
        Returns:
            bool: returns the status of aspect ratio value.
        """
        if manual:
    ...
    def testFunction(self):
        ...
        self.log.testStart(self.testName, '54')
        ...
        for stream in self.testStreams:
            ...
            for pictureMode in self.testtvSettings.getPictureModeInfo():
                for videoFormat in self.testtvSettings.getVideoFormatInfo():
                    aspectRatio = self.testtvSettings.getAspectRatio()  # Get Aspect Ratio

                    self.log.stepStart(f'Set Aspect Ratio: {aspectRatio}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')

                    # Set Picture Mode, Video Format, and Aspect Ratio
                    self.testtvSettings.setAspectRatio(pictureMode, videoFormat, aspectRatio)

                    result = self.testVerifyAspectRatio(pictureMode, videoFormat, aspectRatio, True)

                    self.log.stepResult(result, f'Aspect Ratio: {aspectRatio}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')
...
if __name__ == '__main__':
    test = tvSettings_test54_SaveAspectRatio()
    test.run(False)
