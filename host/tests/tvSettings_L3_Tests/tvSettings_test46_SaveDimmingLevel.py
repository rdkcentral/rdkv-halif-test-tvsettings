...
class tvSettings_test46_SaveTVDimmingMode(utHelperClass):

    testName = "test46_SaveTVDimmingMode"
        ...
        for stream in self.testStreams:
            ...
            for pictureMode in self.testtvSettings.getPictureModeInfo():
                for videoFormat in self.testtvSettings.getVideoFormatInfo():
                    for dimmingLevel in self.testtvSettings.getDimmingLevel():
                        self.log.stepStart(f'Set Dimming Level: {dimmingLevel}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')

                        # Set Picture Mode, Video Format, and Dimming Level
                        self.testtvSettings.setDimmingMode(pictureMode, videoFormat, dimmingLevel)

                        result = self.testVerifyDimmingValue(pictureMode, videoFormat, dimmingLevel, True)

                        self.log.stepResult(result, f'Dimming Level: {dimmingLevel}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')
...
if __name__ == '__main__':
    test = tvSettings_test46_SaveTVDimmingMode()
    test.run(False)
