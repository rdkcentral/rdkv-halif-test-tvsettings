...
class tvSettings_test45_SaveBacklightValues(utHelperClass):

    testName = "test45_SaveBacklightValues"
    ...
    def testVerifyBacklightValue(self, pictureMode, videoFormat, backlight, manual=False):
        """
        Verifies whether the Backlight value is set or not.
            ...
            for pictureMode in self.testtvSettings.getPictureModeInfo():
                for videoFormat in self.testtvSettings.getVideoFormatInfo():
                    for backlight in self.backlightValues:
                        self.log.stepStart(f'Set Backlight Value: {backlight}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')

                        # Set Picture Mode, Video Format, and Backlight Value
                        self.testtvSettings.setPictureMode(pictureMode, videoFormat, backlight)

                        result = self.testVerifyBacklightValue(pictureMode, videoFormat, backlight, True)

                        self.log.stepResult(result, f'Backlight Value: {backlight}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')
...
if __name__ == '__main__':
    test = tvSettings_test45_SaveBacklightValues()
    test.run(False)
