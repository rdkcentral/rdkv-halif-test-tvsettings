...
class tvSettings_test24_Ggain(utHelperClass):

    testName = "test24_Ggain"
    testSetupPath = os.path.join(dir_path, "tvSettings_L3_testSetup.yml")
    moduleName = "tvSettings"
    rackDevice = "dut"
    ggainValues = [0, 512, 1024, 1536, 2047]
    saveSetFlags = [0, 1]  # Save only or set only flags

    def __init__(self):
        """
        Initializes the test24 Ggain test.
    ...
    def testVerifyGgain(self, colorTemperature, ggain, saveSetFlag, manual=False):
        """
        ...
        Returns:
            bool : returns the status of Color Temperature and Ggain
        """
        if manual:
    ...
    def testFunction(self):
        """This function tests the Ggain settings.
            ...
            for colorTemperature in colorTemperatureValues:
                for ggain in self.ggainValues:
                    for saveSetFlag in self.saveSetFlags:
                        self.log.stepStart(f'Color Temperature:{colorTemperature} Ggain:{ggain} Save/Set Flag:{saveSetFlag} Stream:{stream}')

                        # Set the Color Temperature and Ggain
                        self.testtvSettings.setGgainValue(colorTemperature, ggain, saveSetFlag)

                        result = self.testVerifyGgain(colorTemperature, ggain, saveSetFlag, True)

                        self.log.stepResult(result, f'Color Temperature:{colorTemperature} Ggain:{ggain} Save/Set Flag:{saveSetFlag} Stream:{stream}')
