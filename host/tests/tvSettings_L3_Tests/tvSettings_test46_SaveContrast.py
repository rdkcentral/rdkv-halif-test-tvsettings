...
class tvSettings_test46_SaveContrast(tvSettingsHelperClass):
    ...
    def testFunction(self):
        ...
        self.log.testStart(self.testName, '46')  # Start the test with the defined test name
        ...
        for videoFormat, streamUrl in zip(self.testtvSettings.getVideoFormatInfo(), streams):
            ...
            for pictureMode in self.testtvSettings.getPictureModeIndex():
                # Loop through defined contrast values
                for contrast in self.contrastValues:
                    self.log.stepStart(f'Setting Contrast: {contrast}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {streamUrl}')

                    # Set Picture Mode, Video Format, and Contrast Level
                    self.testtvSettings.saveContrastValues(pictureMode, videoFormat, contrast)
                    ...
                    result = self.testVerifyContrastValue(pictureMode, videoFormat, contrast, manual=True)

                    # Log the result for each step
                    self.log.stepResult(result, f'Verification for Contrast: {contrast}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {streamUrl}')
