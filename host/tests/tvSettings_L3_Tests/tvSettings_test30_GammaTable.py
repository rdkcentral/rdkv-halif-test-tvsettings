...
class tvSettings_test30_GammaTable(utHelperClass):

    testName = "test30_GammaTable"
    testSetupPath = os.path.join(dir_path, "tvSettings_L3_testSetup.yml")
    moduleName = "tvSettings"
    gammaSizes = [1, 10, 50, 100, 250]  # 5 specific values from 1 to 255

    def __init__(self):
        """
        Initializes the test30 GammaTable test.
    ...
    def testVerifyGammaTable(self, size, red, green, blue, manual=False):
        ...
        Returns:
            bool : returns the status of Gamma Table settings
        """
        if manual:
    ...
    def testFunction(self):
        ...
        for size in self.gammaSizes:
            # Generate Red, Green, and Blue values based on size
            red = list(range(1, size + 1))  # List of values from 1 to size
            green = list(range(1, size + 1))
            blue = list(range(1, size + 1))
...
if __name__ == '__main__':
    test = tvSettings_test30_GammaTable()
    test.run(False)
