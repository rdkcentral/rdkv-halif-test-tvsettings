...
class tvSettings_test52_SaveHue(utHelperClass):

    testName = "test52_SaveHue"
            ...
            for pictureMode in self.testtvSettings.getPictureModeInfo():
                for videoFormat in self.testtvSettings.getVideoFormatInfo():
                    for hue in self.hueValues:
                        self.log.stepStart(f'Set Hue Value: {hue}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')

                        # Set Picture Mode, Video Format, and Hue Value
                        self.testtvSettings.setHue(pictureMode, videoFormat, hue)

                        result = self.testVerifyHueValue(pictureMode, videoFormat, hue, True)

                        self.log.stepResult(result, f'Hue Value: {hue}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')
...
if __name__ == '__main__':
    test = tvSettings_test52_SaveHue()
    test.run(False)
