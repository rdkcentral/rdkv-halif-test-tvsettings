...
class tvSettings_test50_SaveSharpness(utHelperClass):

    testName = "test50_SaveSharpness"
            ...
            for pictureMode in self.testtvSettings.getPictureModeInfo():
                for videoFormat in self.testtvSettings.getVideoFormatInfo():
                    for sharpness in self.sharpnessValues:
                        self.log.stepStart(f'Set Sharpness Value: {sharpness}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')

                        # Set Picture Mode, Video Format, and Sharpness Value
                        self.testtvSettings.setSharpness(pictureMode, videoFormat, sharpness)

                        result = self.testVerifySharpnessValue(pictureMode, videoFormat, sharpness, True)

                        self.log.stepResult(result, f'Sharpness Value: {sharpness}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')
...
if __name__ == '__main__':
    test = tvSettings_test50_SaveSharpness()
    test.run(False)
