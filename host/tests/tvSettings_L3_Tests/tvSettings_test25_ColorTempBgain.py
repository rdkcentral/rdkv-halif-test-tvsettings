...
class tvSettings_test25_Bgain(utHelperClass):

    testName = "test25_Bgain"
    testSetupPath = os.path.join(dir_path, "tvSettings_L3_testSetup.yml")
    moduleName = "tvSettings"
    rackDevice = "dut"
    bgainValues = [0, 512, 1024, 1536, 2047]
    saveSetFlags = [0, 1]  # Save only or set only flags

    def __init__(self):
        """
        Initializes the test25 Bgain test.
    ...
    def testVerifyBgain(self, colorTemperature, bgain, saveSetFlag, manual=False):
        """
        ...
        Returns:
            bool : returns the status of Color Temperature and Bgain
        """
        if manual:
    ...
    def testFunction(self):
        """This function tests the Bgain settings.
            ...
            for colorTemperature in colorTemperatureValues:
                for bgain in self.bgainValues:
                    for saveSetFlag in self.saveSetFlags:
                        self.log.stepStart(f'Color Temperature:{colorTemperature} Bgain:{bgain} Save/Set Flag:{saveSetFlag} Stream:{stream}')

                        # Set the Color Temperature and Bgain
                        self.testtvSettings.setBgainValue(colorTemperature, bgain, saveSetFlag)

                        result = self.testVerifyBgain(colorTemperature, bgain, saveSetFlag, True)

                        self.log.stepResult(result, f'Color Temperature:{colorTemperature} Bgain:{bgain} Save/Set Flag:{saveSetFlag} Stream:{stream}')
