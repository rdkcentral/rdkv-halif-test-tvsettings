...
class tvSettings_test38_SetGammaPattern(utHelperClass):

    testName = "test38_SetGammaPattern"
    testSetupPath = os.path.join(dir_path, "tvSettings_L3_testSetup.yml")
    moduleName = "tvSettings"
    bitDepths = [0, 1]  # 0 for 10 bit, 1 for 8 bit
    ...
    def __init__(self):
        ...
        Args:
            None.
        """
        super().__init__(self.testName, '38')
    ...
    def testFunction(self):
        """This function tests the Set Gamma Pattern.
        ...
        self.log.testStart(self.testName, '38')
        ...
        for stream in self.testStreams:
            ...
            for bitDepth in self.bitDepths:
                # Define level ranges based on bit depth
                if bitDepth == 0:  # 10 bit
                    redLevel = random.randint(0, 1023)
                    greenLevel = random.randint(0, 1023)
                    blueLevel = random.randint(0, 1023)
                else:  # 8 bit
                    redLevel = random.randint(0, 255)
                    greenLevel = random.randint(0, 255)
                    blueLevel = random.randint(0, 255)

                self.log.stepStart(f'Bit Depth: {bitDepth} Stream: {stream} '
                ...
                self.log.stepResult(result, f'Bit Depth: {bitDepth} Stream: {stream} '
                                            f'Red Level: {redLevel} Green Level: {greenLevel} Blue Level: {blueLevel}')
...
if __name__ == '__main__':
    test = tvSettings_test38_SetGammaPattern()
    test.run(False)
