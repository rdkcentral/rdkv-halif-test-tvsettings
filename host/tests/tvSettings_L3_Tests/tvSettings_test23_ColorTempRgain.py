...
class tvSettings_test23_ColorTempRgain(utHelperClass):

    testName = "test23_ColorTempRgain"
    ...
    def __init__(self):
        """
        Initializes the test23 ColorTempRgain test.
    ...
    def testVerifyColorTempRgain(self, colorTemperature, rgain, saveSetFlag, manual=False):
        ...
        Returns:
            bool : returns the status of Color Temperature and Rgain
        """
        if manual:
    ...
    def testFunction(self):
        """This function tests the ColorTempRgain
            ...
            for colorTemperature in colorTemperatureValues:
                for rgain in self.rgainValues:
                    for saveSetFlag in self.saveSetFlags:
                        self.log.stepStart(f'Color Temperature:{colorTemperature} Rgain:{rgain} Save/Set Flag:{saveSetFlag} Stream:{stream}')

                        # Set the Color Temperature and Rgain
                        self.testtvSettings.setRgainValue(colorTemperature, rgain, saveSetFlag)

                        result = self.testVerifyColorTempRgain(colorTemperature, rgain, saveSetFlag, True)

                        self.log.stepResult(result, f'Color Temperature:{colorTemperature} Rgain:{rgain} Save/Set Flag:{saveSetFlag} Stream:{stream}')
...
if __name__ == '__main__':
    test = tvSettings_test23_ColorTempRgain()
    test.run(False)
