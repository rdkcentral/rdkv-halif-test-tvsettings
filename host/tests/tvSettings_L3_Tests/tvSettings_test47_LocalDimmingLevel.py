...
class tvSettings_test47_SaveLocalDimmingLevel(utHelperClass):

    testName = "test47_SaveLocalDimmingLevel"
    ...
    def testFunction(self):
        """This function tests saving local dimming values with all combinations of picture mode and video format.
        ...
        for stream in self.testStreams:
            ...
            for pictureMode in self.testtvSettings.getPictureModeInfo():
                for videoFormat in self.testtvSettings.getVideoFormatInfo():
                    # Get available local dimming levels
                    localDimmingLevels = self.testtvSettings.getLocalDimmingLevel()

                    for localDimmingLevel in localDimmingLevels:
                        self.log.stepStart(f'Set Local Dimming Level: {localDimmingLevel}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')

                        # Set Picture Mode, Video Format, and Local Dimming Level
                        self.testtvSettings.setLocalDimmingMode(pictureMode, videoFormat, localDimmingLevel)

                        result = self.testVerifyLocalDimmingValue(pictureMode, videoFormat, localDimmingLevel, True)

                        self.log.stepResult(result, f'Local Dimming Level: {localDimmingLevel}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')
...
if __name__ == '__main__':
    test = tvSettings_test47_SaveLocalDimmingLevel()
    test.run(False)
