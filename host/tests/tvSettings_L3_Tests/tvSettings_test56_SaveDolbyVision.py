...
class tvSettings_test56_SaveDolbyVision(utHelperClass):

    testName = "test56_SaveDolbyVision"
        ...
        for stream in self.testStreams:
            ...
            for pictureMode in self.testtvSettings.getPictureModeInfo():
                for videoFormat in self.testtvSettings.getVideoFormatInfo():
                    dolbyVision = self.testtvSettings.getDolbyVisionInfo()  # Get Dolby Vision info

                    self.log.stepStart(f'Set Dolby Vision: {dolbyVision}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')

                    # Set Picture Mode, Video Format, and Dolby Vision Value
                    self.testtvSettings.setDolbyVision(pictureMode, videoFormat, dolbyVision)

                    result = self.testVerifyDolbyVision(pictureMode, videoFormat, dolbyVision, True)

                    self.log.stepResult(result, f'Dolby Vision: {dolbyVision}, Picture Mode: {pictureMode}, Video Format: {videoFormat}, Stream: {stream}')
...
if __name__ == '__main__':
    test = tvSettings_test56_SaveDolbyVision()
    test.run(False)
